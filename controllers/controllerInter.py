...
def add_position_intermediate():
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for('interadm_route'))

        except Exception as e:
            db.session.rollback()
            print(f"Erro ao salvar a posição: {e}")
            flash(f"Erro ao salvar a posição: {e}. Por favor, tente novamente.", 'danger')
            return redirect(url_for('add_position_intermediate'))

    return render_template('adm/posicoes/posicao_inter.html')

# DELETE
def delete_position_intermediate(position_id):
    try:
        position = Position.query.filter_by(id=position_id, level='intermediate').first_or_404()
...
# Atualizar
def edit_position_intermediate(position_id):
    position = Position.query.filter_by(id=position_id, level='intermediate').first_or_404()
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for('edit_position_intermediate', position_id=position_id))
    ...
    return render_template('adm/posicoes/posicao_inter_update.html', position=position)